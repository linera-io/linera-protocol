...
impl Secp256k1PublicKey {
    ...
    pub fn recover_from_hash(
        ...
        VerifyingKey::recover_from_prehash(hash, &signature.0, recovery_id)
...
impl Secp256k1Signature {
    ...
    pub fn from_ethereum_signature(
        ...
        let rid = RecoveryId::from_byte(*v).ok_or(CryptoError::InvalidSecp256k1RecoveryId(*v))?;
...
mod tests {
    ...
    fn parse_ethereum_signature() {
        ...
        for test in test_vectors.iter() {
            ...
            let pubkey = super::Secp256k1PublicKey::recover_from_hash(
                rid,
                &hex::decode(&test.hash_message).unwrap(),
                &signature,
            )
            .unwrap();
