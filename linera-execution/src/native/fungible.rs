...
//!
//! Note: on this branch the `Approve` and `TransferFrom` operations are stubbed
//! with `panic!("not yet implemented")` because the underlying allowance APIs
//! have not yet been backported. The wire format is preserved so the BCS layout
//! matches upstream.
...
///
/// Each query takes a snapshot of the chain's balances and answers from that snapshot,
/// so the GraphQL resolvers don't need to hold a runtime reference. Mutations are
/// appended to a `pending_operations` collector during resolution and scheduled in a
/// single batch after the schema executes.
