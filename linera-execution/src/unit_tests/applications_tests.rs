...
use super::{
    ApplicationRegistry, ApplicationRegistryView, UserApplicationDescription, MultiAddress,
};
...
fn app_id(index: u32) -> MultiAddress {
    MultiAddress {
        module_id: module_id(),
...
fn app_description(index: u32, deps: Vec<u32>) -> UserApplicationDescription {
    UserApplicationDescription {
        ...
        creation: message_id(index),
        parameters: vec![],
        required_applications: deps.into_iter().map(app_id).collect(),
    }
}
