...
impl DynamoDbStore {
    ...
    pub async fn new_for_testing(
        ...
        let (client, table_status) = DynamoDbClient::new_for_testing(kv_config).await?;
        let store = Self {
            client,
    ...
    pub async fn initialize(
        ...
        let client = DynamoDbClient::initialize(kv_config).await?;
        let store = Self {
            client,
            guards: ChainGuards::default(),
            user_applications: Arc::new(DashMap::new()),
    ...
    pub async fn new(
        ...
        let (client, table_status) = DynamoDbClient::new(kv_config).await?;
        let store = Self {
            client,
            guards: ChainGuards::default(),
            user_applications: Arc::new(DashMap::new()),
...
impl DynamoDbStoreClient<TestClock> {
    pub async fn make_test_client(wasm_runtime: Option<WasmRuntime>) -> Self {
        ...
        let (client, _) = DynamoDbStoreClient::new_for_testing(kv_config, wasm_runtime)
            .await
    ...
    pub async fn new_for_testing(
        ...
        let (store, table_status) = DynamoDbStore::new_for_testing(kv_config, wasm_runtime).await?;
...
impl DynamoDbStoreClient<WallClock> {
    pub async fn initialize(
        ...
        let store = DynamoDbStore::initialize(kv_config, wasm_runtime).await?;
        let store_client = DynamoDbStoreClient {
            client: Arc::new(store),
            clock: WallClock,
        };
    ...
    pub async fn new(
        ...
        let (store, table_status) = DynamoDbStore::new(kv_config, wasm_runtime).await?;
        let store_client = DynamoDbStoreClient {
            client: Arc::new(store),
            clock: WallClock,
        };
