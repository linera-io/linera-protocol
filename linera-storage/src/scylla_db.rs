...
impl ScyllaDbStore {
    ...
    pub async fn new_for_testing(
        ...
        let (client, table_status) = ScyllaDbClient::new_for_testing(kv_config).await?;
        let store = Self {
            client,
    ...
    pub async fn initialize(
        ...
        let client = ScyllaDbClient::initialize(kv_config).await?;
        let store = Self {
            client,
            guards: ChainGuards::default(),
            user_applications: Arc::default(),
    ...
    pub async fn new(
        ...
        let (client, table_status) = ScyllaDbClient::new(kv_config).await?;
        let store = Self {
            client,
            guards: ChainGuards::default(),
            user_applications: Arc::default(),
...
impl ScyllaDbStoreClient<TestClock> {
    pub async fn make_test_client(wasm_runtime: Option<WasmRuntime>) -> Self {
        ...
        let (client, _) = ScyllaDbStoreClient::new_for_testing(kv_config, wasm_runtime)
            .await
            .expect("client");
        client
    }
    ...
    pub async fn new_for_testing(
        ...
        let (store, table_status) = ScyllaDbStore::new_for_testing(kv_config, wasm_runtime).await?;
...
impl ScyllaDbStoreClient<WallClock> {
    pub async fn initialize(
        ...
        let store = ScyllaDbStore::initialize(kv_config, wasm_runtime).await?;
        let store_client = ScyllaDbStoreClient {
            client: Arc::new(store),
            clock: WallClock,
        };
    ...
    pub async fn new(
        ...
        let (store, table_status) = ScyllaDbStore::new(kv_config, wasm_runtime).await?;
        let store_client = ScyllaDbStoreClient {
            client: Arc::new(store),
            clock: WallClock,
        };
