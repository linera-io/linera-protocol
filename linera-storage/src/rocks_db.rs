...
impl RocksDbStore {
    ...
    pub async fn new_for_testing(
        ...
        let (client, table_status) = RocksDbClient::new_for_testing(kv_config).await?;
        let store = Self {
            client,
    ...
    pub async fn new(
        ...
        let (client, table_status) = RocksDbClient::new(kv_config).await?;
        let store = Self {
            client,
            guards: ChainGuards::default(),
            user_applications: Arc::default(),
...
impl RocksDbStoreClient<TestClock> {
    pub async fn make_test_client(wasm_runtime: Option<WasmRuntime>) -> Self {
        ...
        let (store_client, _) = RocksDbStoreClient::new_for_testing(kv_config, wasm_runtime)
            .await
            .expect("store_client");
        store_client
    }
    ...
    pub async fn new_for_testing(
        ...
        let (store, table_status) = RocksDbStore::new_for_testing(kv_config, wasm_runtime).await?;
...
impl RocksDbStoreClient<WallClock> {
    pub async fn initialize(
        ...
        let store = RocksDbStore::initialize(kv_config, wasm_runtime).await?;
        let store_client = RocksDbStoreClient {
            client: Arc::new(store),
            clock: WallClock,
        };
    ...
    pub async fn new(
        ...
        let (store, table_status) = RocksDbStore::new(kv_config, wasm_runtime).await?;
        let store_client = RocksDbStoreClient {
            client: Arc::new(store),
            clock: WallClock,
        };
