...
use futures::{
    ...
    stream::{self, AbortHandle, FusedStream, FuturesUnordered, Stream, StreamExt as _},
};
use linera_base::{
    abi::Abi,
...
use serde::Serialize;
use thiserror::Error;
use tokio::sync::{broadcast::Sender as BroadcastSender, Mutex, OwnedRwLockReadGuard};
    ...
    ViewError: From<Storage::StoreError>,
{
    ...
    /// A broadcast sender to notify listeners about new notifications.
    notifications: BroadcastSender<Notification>,
}

const NOTIFICATIONS_BUFFER_SIZE: usize = 1024;
    ...
    ViewError: From<S::StoreError>,
{
    ...
    pub fn new(
        ...
        Self {
            ...
            storage,
            notifications: BroadcastSender::new(NOTIFICATIONS_BUFFER_SIZE),
        }
    }
    ...
    /// Subscribes to all notifications generated by this client.
    pub fn notifications(&self) -> impl Stream<Item = Notification> {
        use tokio_stream::wrappers::{errors::BroadcastStreamRecvError::*, BroadcastStream};
        BroadcastStream::new(self.notifications.subscribe()).filter_map(|notification| {
            future::ready(match notification {
                Ok(notification) => Some(notification),
                Err(Lagged(n)) => {
                    tracing::warn!("receiver too slow: missed {n} messages");
                    None
                }
            })
        })
    ...
    ViewError: From<S::StoreError>,
{
    ...
    fn handle_notifications(&self, notifications: &mut Vec<Notification>) {
        ...
        for notification in notifications.drain(..) {
            if let Err(e) = self.client.notifications.send(notification) {
                tracing::trace!(
                    "no client subscriber available to handle notification {:?}",
                    e.0
                );
            }
        }
