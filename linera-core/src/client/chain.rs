...
use futures::{
    ...
    lock::Mutex,
    stream::{self, AbortHandle, FusedStream, FuturesUnordered, Stream, StreamExt},
};
use dashmap::{
    DashMap,
    mapref::one::{
        Ref as DashMapRef,
        RefMut as DashMapRefMut,
    },
};
use linera_base::{
    abi::Abi,
...
use super::{Client as LineraClient, CommunicationError, LocalNodeError};
...
/// Options and policies that are per chain client.
pub struct Options {
...
impl Default for Options {
    fn default() -> Self {
        Self {
            ...
            cross_chain_message_delivery: CrossChainMessageDelivery::new(false),
        }
    }
}
...
impl<ValidatorNodeProvider, Storage> Client<'_, ValidatorNodeProvider, Storage> {
    pub(super) fn chain(&self) -> DashMapRef<ChainId, super::Chain> {
        self.client.chains.get(&self.chain_id).expect("Chain must not be deleted from client state")
    }

    pub(super) fn chain_mut(&self) -> DashMapRefMut<ChainId, super::Chain> {
        self.client.chains.get_mut(&self.chain_id).expect("Chain must not be deleted from client state")
    }
    ...
    pub fn state_mut(&self) -> impl DerefMut<Target = State> + '_ {
        self.chain_mut().map(|chain| &mut chain.state)
    }
...
// XXX from here is being refactored
...
//         let local_node = LocalNodeClient::new(state, self.notifier.clone());
//         ChainClient {
//             chain_id,
//             known_key_pairs,
...
pub struct Client<'client, ValidatorNodeProvider, Storage> {
    // TODO: use getters here
    // TODO allow setting these on the client
    /// The options for this chain client (not chain).
    pub options: Options,

    /// The protocol [`Client`].
    pub client: &'client LineraClient<ValidatorNodeProvider, Storage>,

    /// The specific chain represented by this [`Client`].  Invariant: this chain
...
impl<P, S> Client<'_, P, S>
where
    P: LocalValidatorNodeProvider + Clone + Sync + 'static,
    ...
    ViewError: From<S::ContextError>,
{
    ...
    //     self.client.local_node.subscribe(vec![self.chain.id]).await
    // }
    ...
    async fn pending_messages(&self) -> Result<Vec<IncomingMessage>, Error> {
    ...
    async fn committees(&self) -> Result<BTreeMap<Epoch, Committee>, LocalNodeError> {
        let (_epoch, committees) = self.epoch_and_committees().await?;
        Ok(committees)
    }
    ...
    async fn known_committees(
        ...
        let (epoch, mut committees) = self.epoch_and_committees().await?;
        let (admin_epoch, admin_committees) = self.client.admin_chain().epoch_and_committees().await?;
    ...
    async fn validator_nodes(&self) -> Result<Vec<(ValidatorName, P::Node)>, Error> {
    ...
    pub async fn identity(&self) -> Result<Owner, Error> {
    ...
    async fn prepare_chain(&self) -> Result<Box<ChainInfo>, Error> {
        ...
        let mut info = self
            .client
            .local_node
            .download_certificates(nodes, self.chain_id, self.state().next_block_height)
            .await?;
        if info.next_block_height == self.state().next_block_height {
        ...
        if ownership
            .all_owners()
            .any(|owner| !self.client.known_key_pairs.contains_key(owner))
        {
    ...
    async fn communicate_chain_updates(
        ...
        let recent_hashed_blobs = self.client.local_node.recent_hashed_blobs().await;
    ...
    async fn communicate_chain_action(
        ...
        let round = match action {
            ...
            Error::ProtocolError("Unexpected response from validators")
        );
        ...
        let certificate = LiteCertificate::try_from_votes(votes)
            .ok_or_else(|| {
                Error::InternalError("Vote values or rounds don't match; this is a bug")
            })?
            .with_value(value)
            .ok_or_else(|| {
                Error::ProtocolError("A quorum voted for an unexpected value")
            })?;
        Ok(certificate)
    }
    ...
    async fn receive_certificate_internal(
        ...
        let (committees, max_epoch) = self.known_committees().await?;
            ...
            Error::CommitteeSynchronizationError
        );
        let remote_committee = committees
            .get(&block.epoch)
            .ok_or_else(|| Error::CommitteeDeprecationError)?;
        ...
        let nodes: Vec<_> = self.client.validator_node_provider.make_nodes(remote_committee)?;
        self.client.local_node
    ...
    async fn receive_certificates_from_validator(
        ...
        // Update tracker.
        self.state_mut().received_certificate_trackers
            .entry(name)
            .and_modify(|t| {
    ...
    async fn find_received_certificates(&self) -> Result<(), Error> {
        // Use network information from the local chain.
        let chain_id = self.chain_id;
        let local_committee = self.local_committee().await?;
        let nodes: Vec<_> = self.client.validator_node_provider.make_nodes(&local_committee)?;
        // Synchronize the state of the admin chain from the network.
        self.client
            .local_node
            .synchronize_chain_state(nodes.clone(), self.client.admin_chain_id)
            .await?;
    ...
    pub async fn transfer(
        ...
        recipient: Recipient,
        user_data: UserData,
    ) -> Result<ClientOutcome<Certificate>, Error> {
    ...
    async fn stage_block_execution_and_discard_failing_messages(
        &self,
        mut block: Block,
    ) -> Result<(ExecutedBlock, ChainInfoResponse), Error> {
        loop {
            let result = self.client.local_node.stage_block_execution(block.clone()).await;
    ...
    async fn propose_block(
        ...
        round: Round,
            ...
            Error::BlockProposalError("Unexpected block height")
        );
        ensure!(
            block.previous_block_hash == self.state().block_hash,
            Error::BlockProposalError("Unexpected previous block hash")
        );
    ...
    async fn set_pending_block(
        ...
        incoming_messages: Vec<IncomingMessage>,
        operations: Vec<Operation>,
    ) -> Result<HashedCertificateValue, Error> {
    ...
    pub async fn query_application(&self, query: Query) -> Result<Response, Error> {
        let response = self
            .client
            .local_node
    ...
    /// `max_pending_messages` incoming messages and the execution fees for a single
    /// block.
    pub async fn query_owner_balance(&self, owner: Owner) -> Result<Amount, Error> {
        Ok(self
    ...
    ///
    /// Does not process the inbox or attempt to synchronize with validators.
    pub async fn local_owner_balance(&self, owner: Owner) -> Result<Amount, Error> {
        Ok(self
    ...
    pub async fn update_validators(&self) -> Result<(), Error> {
        let committee = self.local_committee().await?;
        self.communicate_chain_updates(
            &committee,
            self.chain_id,
    ...
    pub async fn process_pending_block(
        &self,
    ) -> Result<ClientOutcome<Option<Certificate>>, Error> {
        self.find_received_certificates().await?;
        self.prepare_chain().await?;
    ...
    async fn process_pending_block_without_prepare(
        ...
        let maybe_block = manager
            ...
            .cloned()
            .or_else(|| self.state().pending_block.as_ref().cloned());
        ...
        if can_propose {
            ...
            let timestamp = manager.round_timeout.ok_or_else(|| {
                Error::BlockProposalError("Cannot propose in the current round.")
            })?;
    ...
    pub async fn rotate_key_pair(
        ...
        self.client.known_key_pairs.insert(new_public_key.into(), key_pair);
    ...
    pub async fn open_chain(
        ...
        ownership: ChainOwnership,
        balance: Amount,
    ) -> Result<ClientOutcome<(MessageId, Certificate)>, Error> {
        self.prepare_chain().await?;
        loop {
            ...
            let config = OpenChainConfig {
                ...
                committees,
                admin_id: self.client.admin_chain_id,
                epoch,
                balance,
            ...
            let message_id = certificate
                ...
                .ok_or_else(|| Error::InternalError("Failed to create new chain"))?;
            return Ok(ClientOutcome::Committed((message_id, certificate)));
        }
    }

    /// Closes the chain (and loses everything in it!!).
    pub async fn close_chain(&self) -> Result<ClientOutcome<Certificate>, Error> {
    ...
    pub async fn publish_bytecode(
        ...
        contract: Bytecode,
        service: Bytecode,
    ) -> Result<ClientOutcome<(BytecodeId, Certificate)>, Error> {
        self.execute_operation(Operation::System(SystemOperation::PublishBytecode {
            ...
            .try_map(|certificate| {
                ...
                let message_id = certificate
                    ...
                    .ok_or_else(|| Error::InternalError("Failed to publish bytecode"))?;
    ...
    ) -> Result<ClientOutcome<(BlobId, Certificate)>, Error> {
        self.client.local_node.cache_recent_blob(&hashed_blob).await;
        self.state_mut().pending_blobs
    ...
    pub async fn create_application<
        ...
        ) -> Result<ClientOutcome<(UserApplicationId<A>, Certificate)>, Error> {
    ...
    pub async fn create_application_untyped(
        ...
        required_application_ids: Vec<UserApplicationId>,
    ) -> Result<ClientOutcome<(UserApplicationId, Certificate)>, Error> {
        self.execute_operation(Operation::System(SystemOperation::CreateApplication {
            ...
            .try_map(|certificate| {
                ...
                let creation = certificate
                    ...
                    .ok_or_else(|| Error::InternalError("Failed to create application"))?;
    ...
    pub async fn process_inbox(
        &self,
    ) -> Result<(Vec<Certificate>, Option<RoundTimeout>), Error> {
        self.prepare_chain().await?;
        let mut certificates = Vec::new();
        loop {
            let incoming_messages = self.pending_messages().await?;
    ...
    pub async fn process_inbox_if_owned(
        ...
        match self.process_inbox().await {
            Ok(result) => Ok(result),
            Err(Error::CannotFindKeyForChain(_)) => Ok((Vec::new(), None)),
            Err(error) => Err(error),
        }
    }
...
//             (guard.chain_id, nodes, guard.local_node.clone())
//         };
