    ...
    B: StorageBuilder,
{
    let mut builder = TestBuilder::new(storage_builder, 4, 1)
        ...
        .with_policy(ResourceControlPolicy::fuel_and_block());
    let sender = builder.add_root_chain(1, Amount::from_tokens(4)).await?;
    let new_key_pair = SigningKey::generate();
    ...
    B: StorageBuilder,
{
    ...
    let new_owner = SigningKey::generate().public().into();
    let certificate = sender.transfer_ownership(new_owner).await.unwrap().unwrap();
    assert_eq!(sender.next_block_height(), BlockHeight::from(1));
    assert!(sender.pending_proposal().is_none());
    assert_matches!(
        sender.key_pair().await.map(|kp| SigningKey::public(&kp)), // SigningKey isn't Debug; using PublicKey.
    ...
    B: StorageBuilder,
{
    ...
    let key_pair1_b = SigningKey::generate();
    ...
    let key_pair2_b = SigningKey::generate();
    ...
    B: StorageBuilder,
{
    ...
    let key_pair3_b = SigningKey::generate();
    let owner3_b = Owner::from(key_pair3_b.public());
    let key_pair3_c = SigningKey::generate();
