    ...
    S: Storage + Clone + Send + Sync + 'static,
{
    ...
    pub async fn send_chain_information(
        ...
        let start_height: usize = initial_block_height.try_into()?;
        let end_height: usize = target_block_height.try_into()?;
        let (keys, timeout) = {
            let chain = self.local_node.chain_state_view(chain_id).await?;
            let keys = if start_height <= end_height {
                chain.confirmed_log.read(start_height..end_height).await?
            } else {
                Vec::new()
            };
            (keys, chain.manager.timeout.get().clone())
        };
        if !keys.is_empty() {
