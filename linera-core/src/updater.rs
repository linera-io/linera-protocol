    ...
    C: Clock + Sync,
{
    let mut responses: futures::stream::FuturesUnordered<_> = validator_clients
        ...
        .filter_map(|remote_node| {
            ...
            Some(async move {
                ...
                #[cfg(with_metrics)]
                let request_start = clock.instant();
    ...
    'vote_wait: loop {
        ...
        if deadline.is_none() && highest_key_score >= committee.quorum_threshold() {
            let now = clock.instant();
            let grace = now
                .saturating_duration_since(start_time)
                .mul_f64(quorum_grace_period);
    ...
    Env: Environment + 'static,
{
    ...
    async fn send_block_proposal(
        ...
        // One-shot guards: `synced_cross_chain_updates` for the missing-bundles path,
        // `synced_round_and_height` for the round/height mismatch path.
