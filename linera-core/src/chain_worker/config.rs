...
pub struct ChainWorkerConfig {
    ...
    pub key_pair: Option<Arc<Ed25519SecretKey>>,
...
impl ChainWorkerConfig {
    ...
    pub fn with_key_pair(mut self, key_pair: impl Into<Option<Ed25519SecretKey>>) -> Self {
        self.key_pair = key_pair.into().map(Arc::new);
        self
    }

    /// Gets a reference to the [`KeyPair`], if available.
    pub fn key_pair(&self) -> Option<&Ed25519SecretKey> {
