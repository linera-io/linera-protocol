...
fn add_chains(app: &JsValue, data: &Data, new_chains: &[(ChainId, PublicKey)]) {
    let mut chains = data.chains.clone();
    new_chains.iter().for_each(|(chain_id, _)| {
        if !chains.contains(chain_id) {
            chains.push(*chain_id)
        }
    });
    setf(app, "chains", &chains.serialize(&SER).unwrap());
}
                ...
                WsMessage::Text(s) => {
                    ...
                    if let Some(p) = gq.payload {
                        if let Some(d) = p.data {
                            ...
                            if let graphql::Reason::NewBlock {
                                hash: _hash,
                                new_chains,
                                ..
                            } = d.notifications.reason
                            {
                                add_chains(&app, &data, &new_chains);
