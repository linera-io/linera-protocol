...
impl BlockProposal {
    ...
    /// the proposal's own signature for a fresh proposal, the original fast-round
    /// signature for a fast retry, or the authorization retained by the validated
    /// certificate for a regular retry.
    pub fn owner_authorization(&self) -> Option<OwnerAuthorization> {
        match &self.original_proposal {
            None => Some(OwnerAuthorization {
                round: self.content.round,
                signature: self.signature,
            }),
            Some(OriginalProposal::Fast(signature)) => Some(OwnerAuthorization {
                round: Round::Fast,
                signature: *signature,
            }),
            Some(OriginalProposal::Regular { certificate }) => certificate.owner_authorization,
        }
    }
