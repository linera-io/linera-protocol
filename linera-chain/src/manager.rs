    ...
    C: Clone + Context + Send + Sync + 'static,
{
    ...
    pub fallback_owners: RegisterView<C, BTreeMap<Address, u64>>,
}
    ...
    C: Context + Clone + Send + Sync + 'static,
{
    ...
    pub fn reset<'a>(
        ...
        let fallback_owners = fallback_owners
            .map(|(pub_key, weight)| (Address::from(pub_key), weight))
    ...
    fn round_leader(&self, round: Round) -> Option<&Address> {
        match round {
            Round::SingleLeader(r) => {
    ...
    fn is_super(&self, owner: &Address) -> bool {
...
impl ChainManagerInfo {
    ...
    pub fn can_propose(&self, identity: &Address, round: Round) -> bool {
        match round {
