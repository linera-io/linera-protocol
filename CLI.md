...
* `--mixed-self-transfers` — Interleave a self-transfer after every cross-chain transfer, so roughly half the traffic stays on its own chain. Without this a chain only ever sends elsewhere; with `--fan-out 0` it only ever sends to itself. Ignored at `--fan-out 0`
