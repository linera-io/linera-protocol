...
# (Instructions, Total R+W) and cache-dependent metrics (L1/LLC/RAM Hits,
# Estimated Cycles).
#
...
JSON_DIR="${1:?Usage: format-bench-results.sh <json-dir> [--baseline-sha SHA] [--cache-info TEXT]}"
BASELINE_SHA=""
CACHE_INFO=""
shift || true
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --cache-info)   CACHE_INFO="$2"; shift 2 ;;
...
if ! $HAS_CMP; then
  echo "> No baseline available for comparison. Showing absolute counts only. Please rebase onto the base branch to get a comparison."
  echo ""
fi
...
if $HAS_CMP; then
  ...
  for ((j = 0; j < COUNT; j++)); do
    ...
    if [[ -n "$pct" ]] && awk "BEGIN { exit !($pct > 1) }"; then
      REGRESSED_NAMES+=("${B_NAME[j]}")
      REGRESSED_PCTS+=("$pct")
    fi
  done
  if [[ ${#REGRESSED_NAMES[@]} -gt 0 ]]; then
    echo "> [!WARNING]"
    echo "> **${#REGRESSED_NAMES[@]} instruction-count regression(s) detected (>1% threshold):**"
    for ((r = 0; r < ${#REGRESSED_NAMES[@]}; r++)); do
      formatted=$(printf "%+.2f" "${REGRESSED_PCTS[r]}")
      echo "> - \`${REGRESSED_NAMES[r]}\`: ${formatted}%"
    done
    echo ""
  fi
fi
...
echo ""
echo "| Benchmark | L1 Hits | LLC Hits | RAM Hits | Est. Cycles |"
echo "|:---|---:|---:|---:|---:|"
...
echo "> shifts which data lands on which cache lines, changing the L1/LLC/RAM distribution."
...
echo ">"
echo "> Cachegrind simulates a two-level cache (L1 + LLC) auto-detected from the host CPU."
echo "> Est. Cycles = L1 hits + 5 × LLC hits + 35 × RAM hits."
if [[ -n "$CACHE_INFO" ]]; then
  echo ">"
  echo "> **Runner cache sizes:** \`$CACHE_INFO\`"
fi
