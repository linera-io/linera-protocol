...
impl ScyllaDbClientInternal {
    ...
    async fn new_for_testing(
        kv_config: ScyllaDbKvStoreConfig,
    ) -> Result<(Self, TableStatus), ScyllaDbContextError> {
        let session = SessionBuilder::new()
            .known_node(kv_config.uri.as_str())
            .build()
            .await?;
        if Self::test_table_existence(&session, &kv_config.table_name).await? {
            let query = format!("DROP TABLE kv.pairs_{};", kv_config.table_name);
            session.query(query, &[]).await?;
        }
        let stop_if_table_exists = true;
        let create_if_missing = true;
        Self::new_internal(session, kv_config, stop_if_table_exists, create_if_missing).await
    }

    async fn initialize(kv_config: ScyllaDbKvStoreConfig) -> Result<Self, ScyllaDbContextError> {
        let session = SessionBuilder::new()
            .known_node(kv_config.uri.as_str())
            .build()
            .await?;
    ...
    async fn delete_all(kv_config: ScyllaDbKvStoreConfig) -> Result<(), ScyllaDbContextError> {
        let session = SessionBuilder::new()
            .known_node(kv_config.uri.as_str())
            .build()
            .await?;
    ...
    async fn delete_single(kv_config: ScyllaDbKvStoreConfig) -> Result<(), ScyllaDbContextError> {
        let session = SessionBuilder::new()
            .known_node(kv_config.uri.as_str())
            .build()
            .await?;
        let query = format!("DROP TABLE IF EXISTS kv.pairs_{};", kv_config.table_name);
        session.query(query, &[]).await?;
        Ok(())
    }
    ...
    async fn new_internal(
        ...
        let kv_name = format!("{:?}", kv_config);
        let mut existing_table =
            Self::test_table_existence(&session, &kv_config.table_name).await?;
        if !existing_table {
            if create_if_missing {
                existing_table =
                    Self::create_table(&session, &kv_config.table_name, stop_if_table_exists)
                        .await?;
            } else {
        ...
        let client = (session, kv_config.table_name);
        let client = Arc::new(client);
        let semaphore = kv_config
            .common_config
...
impl ScyllaDbClient {
    ...
    pub async fn new_for_testing(
        ...
        let (client, table_status) = ScyllaDbClientInternal::new_for_testing(kv_config).await?;
    ...
    pub async fn initialize(
        ...
        let cache_size = kv_config.common_config.cache_size;
        let client = ScyllaDbClientInternal::initialize(kv_config).await?;
        let client = LruCachingKeyValueClient::new(client, cache_size);
        let client = ScyllaDbClient { client };
        Ok(client)
    }
    ...
    pub async fn delete_all(kv_config: ScyllaDbKvStoreConfig) -> Result<(), ScyllaDbContextError> {
        ScyllaDbClientInternal::delete_all(kv_config).await
    }
    ...
    pub async fn delete_single(
        ...
        ScyllaDbClientInternal::delete_single(kv_config).await
    }
    ...
    pub async fn new(
        ...
        let cache_size = kv_config.common_config.cache_size;
        let (client, table_status) = ScyllaDbClientInternal::new(kv_config).await?;
        let client = LruCachingKeyValueClient::new(client, cache_size);
        let client = ScyllaDbClient { client };
        Ok((client, table_status))
    }
...
pub async fn create_scylla_db_test_client() -> ScyllaDbClient {
    ...
    let (client, _) = ScyllaDbClient::new_for_testing(kv_config)
        .await
        .expect("client");
    client
}
