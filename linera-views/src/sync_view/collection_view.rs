...
mod metrics {
    ...
    pub static COLLECTION_VIEW_HASH_RUNTIME: LazyLock<HistogramVec> = LazyLock::new(|| {
        register_histogram_vec(
            ...
            "SyncCollectionView hash runtime",
            &[],
...
impl<W: SyncClonableView> SyncClonableView for SyncByteCollectionView<W::Context, W> {
...
impl<W: SyncHashableView> SyncHashableView for SyncByteCollectionView<W::Context, W> {
    ...
    fn hash(&self) -> Result<<Self::Hasher as Hasher>::Output, ViewError> {
        ...
        let _hash_latency = metrics::COLLECTION_VIEW_HASH_RUNTIME.measure_latency();
        let mut hasher = sha3::Sha3_256::default();
        let updates = self.updates.read().expect("SyncCollectionView lock should not be poisoned"); // Acquire the lock to prevent writes.
        let keys = self.keys()?;
        let count = keys.len() as u32;
        hasher.update_with_bcs_bytes(&count)?;
        for key in keys {
...
impl<I, W: SyncClonableView> SyncClonableView for SyncCollectionView<W::Context, I, W>
where
    I: Send + Sync + Serialize + DeserializeOwned,
{
    fn clone_unchecked(&mut self) -> Result<Self, ViewError> {
...
impl<I, W: SyncHashableView> SyncHashableView for SyncCollectionView<W::Context, I, W>
where
    I: Send + Sync + Serialize + DeserializeOwned,
{
    type Hasher = sha3::Sha3_256;
...
impl<I: Send + Sync, W: SyncClonableView> SyncClonableView for SyncCustomCollectionView<W::Context, I, W> {
    fn clone_unchecked(&mut self) -> Result<Self, ViewError> {
        Ok(SyncCustomCollectionView {
            collection: self.collection.clone_unchecked()?,
            _phantom: PhantomData,
...
impl<I, W: SyncHashableView> SyncHashableView for SyncCustomCollectionView<W::Context, I, W>
where
    Self: SyncView,
{
    type Hasher = sha3::Sha3_256;
...
/// Type wrapping `SyncByteCollectionView` while memoizing the hash.
pub type SyncHashedByteCollectionView<C, W> =
    SyncWrappedHashableContainerView<C, SyncByteCollectionView<C, W>, HasherOutput>;

/// Wrapper around `SyncByteCollectionView` to compute hashes based on the history of changes.
pub type SyncHistoricallyHashedByteCollectionView<C, W> =
    SyncHistoricallyHashableView<C, SyncByteCollectionView<C, W>>;

/// Type wrapping `SyncCollectionView` while memoizing the hash.
pub type SyncHashedCollectionView<C, I, W> =
    SyncWrappedHashableContainerView<C, SyncCollectionView<C, I, W>, HasherOutput>;

/// Wrapper around `SyncCollectionView` to compute hashes based on the history of changes.
pub type SyncHistoricallyHashedCollectionView<C, I, W> =
    SyncHistoricallyHashableView<C, SyncCollectionView<C, I, W>>;

/// Type wrapping `SyncCustomCollectionView` while memoizing the hash.
pub type SyncHashedCustomCollectionView<C, I, W> =
    SyncWrappedHashableContainerView<C, SyncCustomCollectionView<C, I, W>, HasherOutput>;

/// Wrapper around `SyncCustomCollectionView` to compute hashes based on the history of changes.
pub type SyncHistoricallyHashedCustomCollectionView<C, I, W> =
    SyncHistoricallyHashableView<C, SyncCustomCollectionView<C, I, W>>;
...
mod graphql {
    ...
    where
        K: async_graphql::InputType
            ...
            + std::fmt::Debug,
        V: SyncView + async_graphql::OutputType,
    {
        fn keys(&self) -> Result<Vec<K>, async_graphql::Error> {
            Ok(self.indices()?)
        }
