...
impl<C, T> SyncClonableView for SyncLogView<C, T>
where
    C: Context,
...
impl<C, T> SyncHashableView for SyncLogView<C, T>
where
    C: Context,
...
/// Type wrapping `SyncLogView` while memoizing the hash.
pub type SyncHashedLogView<C, T> = SyncWrappedHashableContainerView<C, SyncLogView<C, T>, HasherOutput>;

/// Wrapper around `SyncLogView` to compute hashes based on the history of changes.
pub type SyncHistoricallyHashedLogView<C, T> = SyncHistoricallyHashableView<C, SyncLogView<C, T>>;
...
mod graphql {
    ...
    impl<C: Context, T: async_graphql::OutputType> SyncLogView<C, T>
    where
