...
//! 1) The first type `SyncKeyValueStoreView` implements SyncView and the function of `KeyValueStore`.
//!
//! 2) The second type `SyncViewContainer` encapsulates `SyncKeyValueStoreView` and provides the following functionalities:
//!    * The `Clone` trait
...
//!
//! Key tags to create the sub-keys of a `SyncKeyValueStoreView` on top of the base key.
...
mod metrics {
    ...
    pub static KEY_VALUE_STORE_VIEW_HASH_LATENCY: LazyLock<HistogramVec> = LazyLock::new(|| {
        register_histogram_vec(
            ...
            "SyncKeyValueStoreView hash latency",
            &[],
    ...
    pub static KEY_VALUE_STORE_VIEW_GET_LATENCY: LazyLock<HistogramVec> = LazyLock::new(|| {
        register_histogram_vec(
            ...
            "SyncKeyValueStoreView get latency",
            &[],
            exponential_bucket_latencies(5.0),
        )
    });
    ...
    pub static KEY_VALUE_STORE_VIEW_MULTI_GET_LATENCY: LazyLock<HistogramVec> =
        LazyLock::new(|| {
            register_histogram_vec(
                ...
                "SyncKeyValueStoreView multi get latency",
                &[],
                exponential_bucket_latencies(5.0),
            )
        });
    ...
    pub static KEY_VALUE_STORE_VIEW_CONTAINS_KEY_LATENCY: LazyLock<HistogramVec> =
        LazyLock::new(|| {
            register_histogram_vec(
                ...
                "SyncKeyValueStoreView contains key latency",
                &[],
                exponential_bucket_latencies(5.0),
            )
        });
    ...
    pub static KEY_VALUE_STORE_VIEW_CONTAINS_KEYS_LATENCY: LazyLock<HistogramVec> =
        LazyLock::new(|| {
            register_histogram_vec(
                ...
                "SyncKeyValueStoreView contains keys latency",
                &[],
                exponential_bucket_latencies(5.0),
            )
        });
    ...
    pub static KEY_VALUE_STORE_VIEW_FIND_KEYS_BY_PREFIX_LATENCY: LazyLock<HistogramVec> =
        LazyLock::new(|| {
            register_histogram_vec(
                ...
                "SyncKeyValueStoreView find keys by prefix latency",
                &[],
                exponential_bucket_latencies(5.0),
            )
        });
    ...
    pub static KEY_VALUE_STORE_VIEW_FIND_KEY_VALUES_BY_PREFIX_LATENCY: LazyLock<HistogramVec> =
        LazyLock::new(|| {
            register_histogram_vec(
                ...
                "SyncKeyValueStoreView find key values by prefix latency",
                &[],
                exponential_bucket_latencies(5.0),
            )
        });
    ...
    pub static KEY_VALUE_STORE_VIEW_WRITE_BATCH_LATENCY: LazyLock<HistogramVec> =
        LazyLock::new(|| {
            register_histogram_vec(
                ...
                "SyncKeyValueStoreView write batch latency",
                &[],
                exponential_bucket_latencies(5.0),
            )
        });
...
#[cfg(with_testing)]
use {
    crate::store::{KeyValueStoreError, WithError, WritableSyncKeyValueStore},
    std::sync::RwLock,
    std::sync::Arc,
    thiserror::Error,
};
...
#[allocative(bound = "C")]
pub struct SyncKeyValueStoreView<C> {
    /// The view context.
    #[allocative(skip)]
    context: C,
...
impl<C: Context, C2: Context> SyncReplaceContext<C2> for SyncKeyValueStoreView<C> {
    type Target = SyncKeyValueStoreView<C2>;
...
impl<C: Context> SyncView for SyncKeyValueStoreView<C> {
    const NUM_INIT_KEYS: usize = 2 + SyncByteMapView::<C, u32>::NUM_INIT_KEYS;
    ...
    fn pre_load(context: &C) -> Result<Vec<Vec<u8>>, ViewError> {
        ...
        v.extend(SyncByteMapView::<C, u32>::pre_load(&context_sizes)?);
        Ok(v)
    }
    ...
    fn post_load(context: C, values: &[Option<Vec<u8>>]) -> Result<Self, ViewError> {
        ...
        let base_key = context.base_key().base_tag(KeyTag::Sizes as u8);
        let context_sizes = context.clone_with_base_key(base_key);
        let sizes = SyncByteMapView::post_load(
            context_sizes,
    ...
    fn has_pending_changes(&self) -> bool {
        ...
        if self.sizes.has_pending_changes() {
            return true;
        }
        let hash = self.hash.lock().unwrap();
...
impl<C: Context> SyncClonableView for SyncKeyValueStoreView<C> {
    fn clone_unchecked(&mut self) -> Result<Self, ViewError> {
        Ok(SyncKeyValueStoreView {
...
impl<C: Context> SyncKeyValueStoreView<C> {
    fn max_key_size(&self) -> usize {
        let prefix_len = self.context.base_key().bytes.len();
        <C::Store as ReadableSyncKeyValueStore>::MAX_KEY_SIZE - 1 - prefix_len
    }

    /// Getting the total sizes that will be used for keys and values when stored
    /// ```rust
    /// # use linera_views::context::SyncMemoryContext;
    /// # use linera_views::sync_view::key_value_store_view::{SyncKeyValueStoreView, SizeData};
    /// # use linera_views::sync_view::SyncView;
    /// # let context = SyncMemoryContext::new_for_testing(());
    /// let mut view = SyncKeyValueStoreView::load(context).unwrap();
    /// view.insert(vec![0, 1], vec![0, 1, 2, 3, 4]).unwrap();
    ...
    /// ```rust
    /// # use linera_views::context::SyncMemoryContext;
    /// # use linera_views::sync_view::key_value_store_view::SyncKeyValueStoreView;
    /// # use linera_views::sync_view::SyncView;
    /// # let context = SyncMemoryContext::new_for_testing(());
    /// let mut view = SyncKeyValueStoreView::load(context).unwrap();
    ...
    /// view.insert(vec![0, 1], vec![0]).unwrap();
    /// view.insert(vec![0, 2], vec![0]).unwrap();
    /// let key_values = view.indices().unwrap();
    ...
    /// assert_eq!(view.get(&[0, 1]).unwrap(), Some(vec![42]));
    /// assert_eq!(view.get(&[0, 2]).unwrap(), None);
    /// ```
    pub fn get(&self, index: &[u8]) -> Result<Option<Vec<u8>>, ViewError> {
        ...
        ensure!(index.len() <= self.max_key_size()::KeyTooLong);
        ...
        let key = self
            ...
            .base_key()
            .base_tag_index(KeyTag::Index as u8, index);
        Ok(self.context.store().read_value_bytes(&key)?)
    }
    ...
    /// let results = view.contains_keys(&keys).unwrap();
    ...
    pub fn contains_keys(&self, indices: &[Vec<u8>]) -> Result<Vec<bool>, ViewError> {
        ...
        let values = self.context.store().contains_keys(&vector_query)?;
    ...
    /// view.insert(vec![0, 1], vec![42]).unwrap();
    /// assert_eq!(
    ///     view.multi_get(&[vec![0, 1], vec![0, 2]]).unwrap(),
    ...
    /// let key_values = view.find_key_values_by_prefix(&[0]).unwrap();
        ...
        for operation in batch.operations {
            match operation {
                WriteOperation::Delete { key } => {
                    ensure!(key.len() <= max_key_size::KeyTooLong);
                ...
                WriteOperation::DeletePrefix { key_prefix } => {
                    ensure!(key_prefix.len() <= max_key_size::KeyTooLong);
                    let key_list = self
                        .updates
                    ...
                    let key_values = self.sizes.key_values_by_prefix(key_prefix.clone())?;
    ...
    /// let mut view = SyncKeyValueStoreView::load(context).unwrap();
    /// view.insert(vec![0, 1], vec![34]).unwrap();
    /// assert_eq!(view.get(&[0, 1]).unwrap(), Some(vec![34]));
    /// ```
    ...
    /// assert_eq!(view.get(&[0, 1]).unwrap(), None);
    /// ```
    ...
    /// view.insert(vec![0, 1], vec![34]).unwrap();
    /// view.insert(vec![3, 4], vec![42]).unwrap();
    /// let keys = view.find_keys_by_prefix(&[0]).unwrap();
    ...
    pub fn find_keys_by_prefix(&self, key_prefix: &[u8]) -> Result<Vec<Vec<u8>>, ViewError> {
        ...
        let _latency = metrics::KEY_VALUE_STORE_VIEW_FIND_KEYS_BY_PREFIX_LATENCY.measure_latency();
            ...
            key_prefix.len() <= self.max_key_size()::KeyTooLong
        );
...
impl<C: Context> SyncHashableView for SyncKeyValueStoreView<C> {
...
/// Type wrapping `SyncKeyValueStoreView` while memoizing the hash.
pub type SyncHashedKeyValueStoreView<C> =
    SyncWrappedHashableContainerView<C, SyncKeyValueStoreView<C>, HasherOutput>;

/// Wrapper around `SyncKeyValueStoreView` to compute hashes based on the history of changes.
pub type SyncHistoricallyHashedKeyValueStoreView<C> = SyncHistoricallyHashableView<C, SyncKeyValueStoreView<C>>;

/// A virtual DB client using a `SyncKeyValueStoreView` as a backend (testing only).
#[cfg(with_testing)]
#[derive(Debug, Clone)]
pub struct SyncViewContainer<C> {
    view: Arc<RwLock<SyncKeyValueStoreView<C>>>,
}

#[cfg(with_testing)]
impl<C> WithError for SyncViewContainer<C> {
    type Error = ViewContainerError;
}

#[cfg(with_testing)]
/// The error type for [`SyncViewContainer`] operations.
#[derive(Error, Debug)]
...
#[cfg(with_testing)]
impl<C: Context> ReadableSyncKeyValueStore for SyncViewContainer<C> {
    const MAX_KEY_SIZE: usize = <C::Store as ReadableSyncKeyValueStore>::MAX_KEY_SIZE;
    ...
    fn read_value_bytes(&self, key: &[u8]) -> Result<Option<Vec<u8>>, ViewContainerError> {
        let view = self.view.read().expect("SyncViewContainer lock should not be poisoned");
        Ok(view.get(key)?)
    }

    fn contains_key(&self, key: &[u8]) -> Result<bool, ViewContainerError> {
        let view = self.view.read().expect("SyncViewContainer lock should not be poisoned");
        Ok(view.contains_key(key)?)
    }

    fn contains_keys(&self, keys: &[Vec<u8>]) -> Result<Vec<bool>, ViewContainerError> {
        let view = self.view.read().expect("SyncViewContainer lock should not be poisoned");
        Ok(view.contains_keys(keys)?)
    }
    ...
    fn find_keys_by_prefix(
        ...
        let view = self.view.read().expect("SyncViewContainer lock should not be poisoned");
        Ok(view.find_keys_by_prefix(key_prefix)?)
    }
    ...
    fn find_key_values_by_prefix(
        ...
        let view = self.view.read().expect("SyncViewContainer lock should not be poisoned");
        Ok(view.find_key_values_by_prefix(key_prefix)?)
    }
}

#[cfg(with_testing)]
impl<C: Context> WritableSyncKeyValueStore for SyncViewContainer<C> {
    const MAX_VALUE_SIZE: usize = <C::Store as WritableSyncKeyValueStore>::MAX_VALUE_SIZE;

    fn write_batch(&self, batch: Batch) -> Result<(), ViewContainerError> {
        let mut view = self.view.write().expect("SyncViewContainer lock should not be poisoned");
    ...
    fn clear_journal(&self) -> Result<(), ViewContainerError> {
        Ok(())
    }
}

#[cfg(with_testing)]
impl<C: Context> SyncViewContainer<C> {
    /// Creates a [`SyncViewContainer`].
    pub fn new(context: C) -> Result<Self, ViewError> {
        let view = SyncKeyValueStoreView::load(context)?;
