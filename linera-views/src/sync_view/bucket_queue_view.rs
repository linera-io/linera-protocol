...
mod metrics {
    ...
    pub static BUCKET_QUEUE_VIEW_HASH_RUNTIME: LazyLock<HistogramVec> = LazyLock::new(|| {
        register_histogram_vec(
            ...
            "SyncBucketQueueView hash runtime",
            &[],
...
/// Key tags to create the sub-keys of a [`SyncBucketQueueView`] on top of the base key.
#[repr(u8)]
enum KeyTag {
...
pub struct SyncBucketQueueView<C, T, const N: usize> {
    /// The view context.
    #[allocative(skip)]
    context: C,
...
impl<C, T, const N: usize> SyncView for SyncBucketQueueView<C, T, N>
where
    C: Context,
    T: Send + Sync + Clone + Serialize + DeserializeOwned,
{
    ...
    fn post_load(context: C, values: &[Option<Vec<u8>>]) -> Result<Self, ViewError> {
        ...
        // TODO(#4969): Remove redundant BucketDescription in SyncBucketQueueView.
...
impl<C: Clone, T: Clone, const N: usize> SyncClonableView for SyncBucketQueueView<C, T, N>
where
    Self: SyncView,
{
    fn clone_unchecked(&mut self) -> Result<Self, ViewError> {
        Ok(SyncBucketQueueView {
...
impl<C: Context, T, const N: usize> SyncBucketQueueView<C, T, N> {
    ...
    /// # use linera_views::sync_view::bucket_queue_view::SyncBucketQueueView;
    /// # use crate::linera_views::sync_view::SyncView;
    /// # let context = SyncMemoryContext::new_for_testing(());
    /// let mut queue = SyncBucketQueueView::<_, u8, 5>::load(context).unwrap();
    /// queue.push_back(34);
...
impl<C: Context, T: DeserializeOwned + Clone, const N: usize> SyncBucketQueueView<C, T, N> {
    ...
    /// # use crate::linera_views::sync_view::SyncView;
    /// # let context = SyncMemoryContext::new_for_testing(());
    /// let mut queue = SyncBucketQueueView::<_, u128, 5>::load(context).unwrap();
    /// queue.push_back(34 as u128);
    /// queue.delete_front().unwrap();
        ...
        match self.cursor {
            Some(cursor) => {
                ...
                if offset == self.stored_buckets.len() {
                    ...
                    if !bucket.is_loaded() {
                        let key = self.get_bucket_key(index)?;
                        let data = self.context.store().read_value(&key)?;
                        let data = match data {
                            Some(value) => value,
                            None => {
                                return Err(ViewError::MissingEntries(
                                    "SyncBucketQueueView::delete_front".into(),
                                ));
                            }
                        };
    ...
    /// let mut queue = SyncBucketQueueView::<_, u128, 5>::load(context).unwrap();
    /// queue.push_back(34);
    /// assert_eq!(queue.elements().unwrap(), vec![34]);
    /// ```
    ...
    pub fn elements(&self) -> Result<Vec<T>, ViewError> {
        ...
        self.read_context(self.cursor, count)
    }
    ...
    /// queue.push_back(34);
    /// queue.push_back(37);
    /// assert_eq!(queue.back().unwrap(), Some(37));
    /// ```
        ...
        T: Clone,
    {
        ...
        if !bucket.is_loaded() {
            ...
            let data = match data {
                Some(data) => data,
                None => {
                    return Err(ViewError::MissingEntries("SyncBucketQueueView::back".into()));
                }
            };
    ...
    fn read_context(
        ...
        if let Some(cursor) = cursor {
            ...
            let values = self.context.store().read_multi_values_bytes(&keys)?;
            let mut value_pos = 0;
            count_remain = count;
            let mut position = cursor.position;
            for offset in cursor.offset..self.stored_buckets.len() {
                ...
                let data = match &bucket.state {
                    ...
                    State::NotLoaded { .. } => {
                        let value = match &values[value_pos] {
                            ...
                            None => {
                                return Err(ViewError::MissingEntries(
                                    "SyncBucketQueueView::read_context".into(),
                                ));
                            }
                        };
                        value_pos += 1;
    ...
    /// queue.push_back(37);
    /// queue.push_back(47);
    /// assert_eq!(queue.read_front(2).unwrap(), vec![34, 37]);
    /// ```
    ...
    /// queue.push_back(37);
    /// queue.push_back(47);
    /// assert_eq!(queue.read_back(2).unwrap(), vec![37, 47]);
    /// ```
...
impl<C: Context, T: Serialize + DeserializeOwned + Send + Sync + Clone, const N: usize> SyncHashableView
    for SyncBucketQueueView<C, T, N>
where
    Self: SyncView,
{
...
pub type SyncHashedBucketQueueView<C, T, const N: usize> =
    SyncWrappedHashableContainerView<C, SyncBucketQueueView<C, T, N>, HasherOutput>;

/// Wrapper around `SyncBucketQueueView` to compute hashes based on the history of changes.
pub type SyncHistoricallyHashedBucketQueueView<C, T, const N: usize> =
    SyncHistoricallyHashableView<C, SyncBucketQueueView<C, T, N>>;

#[cfg(with_graphql)]
mod graphql {
    use std::borrow::Cow;

    use super::SyncBucketQueueView;
    use crate::{
        context::Context,
    ...
    impl<C: Context, T: async_graphql::OutputType, const N: usize> SyncBucketQueueView<C, T, N>
    where
        C: Send + Sync,
