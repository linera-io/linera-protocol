...
use crate::{
    ...
    dynamo_db::{create_dynamo_db_test_config, DynamoDbStore},
};
...
async fn table_is_created() {
    ...
    DynamoDbStore::delete_all(&config)
        .await
        .expect("successful cleaning");
    let namespace = "lineratableiscreated".to_string();
    let initial_tables = DynamoDbStore::list_all(&config)
        .await
        .expect("initial_tables");
    assert!(!initial_tables.contains(&namespace));

    DynamoDbStore::create(&config, &namespace)
        .await
...
async fn separate_tables_are_created() {
    ...
    DynamoDbStore::create(&config, &namespace1)
        .await
        .expect("to create namespace1");
    DynamoDbStore::create(&config, &namespace2)
        .await
        .expect("to create namespace2");
