...
impl KeyValueStoreMetrics {
    ...
    pub fn new(name: &str) -> Self {
        ...
        let entry1 = format!("{var_name}_open_shared_latency");
        let entry2 = format!("{title_name} open shared partition");
        let open_shared_latency =
            register_histogram_vec(&entry1, &entry2, &[], latency_buckets.clone());

        let entry1 = format!("{var_name}_open_exclusive_latency");
        let entry2 = format!("{title_name} open exclusive partition");
