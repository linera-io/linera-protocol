...
struct ScyllaDbClient {
    ...
    multi_key_ps: DashMap<usize, PreparedStatement>,
}
...
impl ScyllaDbClient {
    async fn get_multi_stmt(
        ...
        let cache = if selects_value {
            &self.multi_kv_ps
        } else {
            &self.multi_key_ps
        };
        if let Some(entry) = cache.get(&num_markers) {
            return Ok(entry.clone());
        }
        let markers = std::iter::repeat_n("?", num_markers)
            .collect::<Vec<_>>()
            .join(",");
        let query = if selects_value {
            format!(
        ...
        cache.insert(num_markers, prepared_statement.clone());
    ...
    async fn new(session: Session, namespace: &str) -> Result<Self, ScyllaDbStoreInternalError> {
        ...
        Ok(Self {
            ...
            multi_kv_ps: DashMap::new(),
            multi_key_ps: DashMap::new(),
        })
    }
    ...
    async fn read_multi_values_internal(
        ...
        let ps = self.get_multi_stmt(true, map.len()).await?;
        let mut inputs = vec![root_key.to_vec()];
        inputs.extend(map.keys().cloned());
        let mut rows = self
            .session
            .execute_iter(ps, &inputs)
            .await?
    ...
    async fn contains_keys_internal(
        ...
        let map = Self::get_occurences_map(keys)?;
        let ps = self.get_multi_stmt(false, map.len()).await?;
        let mut inputs = vec![root_key.to_vec()];
        inputs.extend(map.keys().cloned());
        let mut rows = self
            .session
...
async fn build_session(uri: &str) -> Result<Session, ScyllaDbStoreInternalError> {
    let lbp = DefaultPolicy::builder().token_aware(true).build();
...
impl AdminKeyValueStore for ScyllaDbStoreInternal {
    ...
    async fn list_all(config: &Self::Config) -> Result<Vec<String>, ScyllaDbStoreInternalError> {
        let session = build_session(&config.uri).await?;
        let prepared_query = session.prepare("DESCRIBE KEYSPACE kv").await?;
        let result = session.execute_iter(prepared_query, &[]).await;
    ...
    async fn list_root_keys(
        ...
        // Execute the query
        let rows = session.execute_iter(prepared_query, &[]).await?;
        let mut rows = rows.rows_stream::<(Vec<u8>,)>()?;
