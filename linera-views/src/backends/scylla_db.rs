...
#[cfg(with_metrics)]
use crate::metering::{MeteredDatabase, MeteredStore};
#[cfg(with_testing)]
use crate::store::TestKeyValueDatabase;
use crate::{
    batch::UnorderedBatch,
    common::{get_uleb128_size, get_upper_bound_option},
    journaling::{JournalConsistencyError, JournalingKeyValueDatabase, JournalingKeyValueStore},
    lru_caching::{LruCachingConfig, LruCachingDatabase, LruCachingStore},
    store::{
        DirectWritableKeyValueStore, KeyValueDatabase, KeyValueStoreError, ReadableKeyValueStore,
        WithError,
    },
    value_splitting::{ValueSplittingDatabase, ValueSplittingError, ValueSplittingStore},
    FutureSyncExt as _,
};
...
#[cfg(with_testing)]
impl ScyllaDbStore {
    /// Creates a `ScyllaDbStore` for testing purposes.
    pub async fn new_test_store() -> Result<Self, ScyllaDbStoreError> {
        let database = ScyllaDbDatabase::connect_test_namespace().await?;
        database.open_exclusive(&[])
    }
}
