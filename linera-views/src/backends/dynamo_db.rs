...
impl DynamoDbStoreInternal {
    async fn write_root_key(&self, root_key: &[u8]) -> Result<(), DynamoDbStoreInternalError> {
...
impl AdminKeyValueStore for DynamoDbStoreInternal {
    ...
    async fn connect(
        ...
        store.write_root_key(root_key).await?;
        Ok(store)
    }
    ...
    async fn clone_with_root_key(
        ...
        self.write_root_key(root_key).await?;
    ...
    async fn list_root_keys(
        ...
        store.start_key = vec![1];

        let key_prefix = &[0];
        let keys = store.find_keys_by_prefix(key_prefix).await?;
