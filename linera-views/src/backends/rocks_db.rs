...
#[cfg(with_testing)]
impl RocksDbStore {
    /// Creates a `RocksDbStore` for testing purposes.
    pub async fn new_test_store() -> Result<Self, RocksDbStoreError> {
        let database = RocksDbDatabase::connect_test_namespace().await?;
