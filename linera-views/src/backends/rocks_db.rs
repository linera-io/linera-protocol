...
impl RocksDbStoreExecutor {
    pub fn contains_keys_internal(
        ...
        for (i, key) in keys.into_iter().enumerate() {
            check_key_size(&key)?;
            let mut full_key = self.effective_root_key.to_vec();
            full_key.extend(key);
    ...
    fn find_keys_by_prefix_internal(
        ...
        let mut prefix = self.effective_root_key.clone();
            ...
            match operation {
                ...
                WriteOperation::DeletePrefix { key_prefix } => {
                    ...
                    if let Excluded(upper_bound) = get_upper_bound(&key_prefix) {
                        let mut full_key1 = self.effective_root_key.to_vec();
                        full_key1.extend(key_prefix);
                        let mut full_key2 = self.effective_root_key.to_vec();
...
impl ReadableKeyValueStore for RocksDbStoreInternal {
    ...
    async fn read_value_bytes(
        ...
        let mut full_key = self.executor.effective_root_key.to_vec();
        full_key.extend(key);
        self.spawn_mode
