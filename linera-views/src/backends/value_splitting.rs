    ...
    K::Error: 'static,
{
    ...
    async fn read_value_bytes(&self, key: &[u8]) -> Result<Option<Vec<u8>>, Self::Error> {
        ...
        let value = self.inner.read_value_bytes(&big_key).await?;
        ...
        let segments = self.inner.read_multi_values_bytes(big_keys).await?;
    ...
    async fn contains_key(&self, key: &[u8]) -> Result<bool, Self::Error> {
        let mut big_key = key.to_vec();
        big_key.extend(&[0, 0, 0, 0]);
        Ok(self.inner.contains_key(&big_key).await?)
    }
    ...
    async fn contains_keys(&self, keys: Vec<Vec<u8>>) -> Result<Vec<bool>, Self::Error> {
        let big_keys = keys
            ...
            .collect::<Vec<_>>();
        Ok(self.inner.contains_keys(big_keys).await?)
    }
    ...
    async fn read_multi_values_bytes(
        ...
        let values = self.inner.read_multi_values_bytes(big_keys).await?;
    ...
    async fn find_keys_by_prefix(&self, key_prefix: &[u8]) -> Result<Vec<Vec<u8>>, Self::Error> {
        ...
        for big_key in self.inner.find_keys_by_prefix(key_prefix).await? {
            let len = big_key.len();
    ...
    async fn find_key_values_by_prefix(
        ...
        let small_key_values = self.inner.find_key_values_by_prefix(key_prefix).await?;
    ...
    K::Error: 'static,
{
    ...
    async fn write_batch(&self, batch: Batch) -> Result<(), Self::Error> {
        ...
        Ok(self.inner.write_batch(batch_new).await?)
    }

    async fn clear_journal(&self) -> Result<(), Self::Error> {
        Ok(self.inner.clear_journal().await?)
    }
}
    ...
    D: WithError,
{
    ...
    pub fn new(store: D) -> Self {
        ValueSplittingStore { inner: store }
    }
