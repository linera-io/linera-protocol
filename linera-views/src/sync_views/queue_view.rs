...
{
    fn get(&self, index: u32) -> Result<Option<T>, ViewError> {
        let key = self
            .0
            .context
            .base_key()
    ...
    fn read_context(&self, range: Range<u32>) -> Result<Vec<T>, ViewError> {
        let count = range.len();
    ...
    pub fn read_back(&self, mut count: usize) -> Result<Vec<T>, ViewError> {
        ...
        if count <= new_back_len || self.0.delete_storage_first {
            ...
            let start = self.0.stored_indices.end - stored_consumed;
            values.extend(self.read_context(start..self.0.stored_indices.end)?);
    ...
    fn load_all(&mut self) -> Result<(), ViewError> {
        if !self.0.delete_storage_first {
            ...
            self.0.new_back_values.rotate_right(shift as usize);
