...
use crate::{
    batch::Batch,
    common::{
        from_bytes_option, get_key_range_for_prefix, CustomSerialize, SuffixClosedSetIterator,
        Update,
    },
...
///
/// This is a thin wrapper around [`ByteMapView`] that implements [`SyncView`]
    ...
    V: Send + Sync + Serialize,
{
    ...
    fn post_load(context: C, _values: &[Option<Vec<u8>>]) -> Result<Self, ViewError> {
        Ok(SyncByteMapView(ByteMapView::base_post_load(context)))
    }
    ...
    C: SyncContext,
{
    ...
    pub fn remove_by_prefix(&mut self, key_prefix: Vec<u8>) {
        self.0.remove_by_prefix(key_prefix);
    }
    ...
    pub fn contains_key(&self, short_key: &[u8]) -> Result<bool, ViewError> {
        ...
        if self.0.deletion_set.contains_prefix_of(short_key) {
            return Ok(false);
        }
    ...
    V: Clone + DeserializeOwned + 'static,
{
    ...
    pub fn multi_get(&self, short_keys: Vec<Vec<u8>>) -> Result<Vec<Option<V>>, ViewError> {
        ...
        for (i, short_key) in short_keys.into_iter().enumerate() {
            ...
            } else if !self.0.deletion_set.contains_prefix_of(&short_key) {
    ...
    pub fn get_mut(&mut self, short_key: &[u8]) -> Result<Option<&mut V>, ViewError> {
        let update = match self.0.updates.entry(short_key.to_vec()) {
            Entry::Vacant(e) => {
                if self.0.deletion_set.contains_prefix_of(short_key) {
                    None
                } else {
                    let key = self.0.context.base_key().base_index(short_key);
                    let value = self.0.context.store().read_value(&key)?;
    ...
    V: Clone + Serialize + DeserializeOwned + 'static,
{
        ...
        F: FnMut(&[u8]) -> Result<bool, ViewError> + Send,
    {
        ...
        if !self.0.deletion_set.contains_prefix_of(prefix) {
            let iter = self
                .0
                .deletion_set
                .deleted_prefixes
            ...
            let base = self.0.context.base_key().base_index(prefix);
        ...
        F: FnMut(&[u8], ValueOrBytes<'a, V>) -> Result<bool, ViewError> + Send,
    {
        ...
        if !self.0.deletion_set.contains_prefix_of(prefix) {
            ...
            let mut suffix_closed_set = SuffixClosedSetIterator::new(prefix_len, iter);
            let base = self.0.context.base_key().base_index(prefix);
            for (index, bytes) in self.0.context.store().find_key_values_by_prefix(&base)? {
                loop {
                    match update {
    ...
    V: Default + DeserializeOwned + 'static,
{
    ...
    pub fn get_mut_or_default(&mut self, short_key: &[u8]) -> Result<&mut V, ViewError> {
        let update = match self.0.updates.entry(short_key.to_vec()) {
            Entry::Vacant(e) if self.0.deletion_set.contains_prefix_of(short_key) => {
                e.insert(Update::Set(V::default()))
            }
            Entry::Vacant(e) => {
                let key = self.0.context.base_key().base_index(short_key);
                let value = self.0.context.store().read_value(&key)?.unwrap_or_default();
...
/// A synchronous `View` that has a type for keys. The ordering of the entries
...
/// A synchronous map view that uses custom serialization for keys.
#[derive(Debug, Allocative)]
#[allocative(bound = "C, I, V: Allocative")]
...
mod graphql {
            ...
            + 'static,
    {
        ...
        async fn count_(&self) -> Result<u32, async_graphql::Error> {
            u32::try_from(self.count()?).map_err(|_| async_graphql::Error::new("count exceeds u32"))
        }
