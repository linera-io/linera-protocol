...
use fungible_graphql::{
    Account, AccountOwner, ApplicationCall, Destination, Effect, SessionCall, Transfer,
};
use linera_sdk::{
...
use thiserror::Error;
use linera_sdk::contract::system_api::ViewStorageContext;
...
impl Contract for FungibleToken<ViewStorageContext> {
    ...
    async fn initialize(
        ...
        let mut accounts: BTreeMap<AccountOwner, Amount>  = bcs::from_bytes(argument).map_err(Error::InvalidInitialState)?;
        if let Some(owner) = context.authenticated_signer {
            accounts.insert(AccountOwner::with_user(owner), Amount::from_str("1000000").unwrap());
        }
    ...
    async fn execute_operation(
        ...
        let transfer = Transfer::from_bcs_bytes(operation).map_err(Error::InvalidOperation)?;
        let Transfer {
            owner,
            amount,
            target_account,
...
impl FungibleToken<ViewStorageContext> {
    ...
    fn check_account_authentication(
        ...
        if owner.user() == authenticated_signer.as_ref() {
            return Ok(());
        }
        if owner.application() == authenticated_application_id.as_ref() {
            return Ok(());
        }
