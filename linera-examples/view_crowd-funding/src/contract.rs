...
#[async_trait]
impl<C: Context + Send + Sync + Clone + 'static> Contract for ViewCrowdFunding<C>
where
...
impl<C: Context + Send + Sync + Clone + 'static> ViewCrowdFunding<C>
where
    ViewError: From<<C as linera_views::common::Context>::Error>,
{
    ...
    async fn signed_pledge(&mut self, transfer: SignedTransfer) -> Result<(), Error> {
        ...
        self.transfer(view_fungible::ApplicationCall::Delegated(transfer))
            .await?;
    ...
    async fn query_session_balances(sessions: &[SessionId]) -> Result<Vec<u128>, Error> {
        let balance_query = bcs::to_bytes(&view_fungible::SessionCall::Balance)
    ...
    async fn balance(&self) -> Result<u128, Error> {
        let query_bytes = bcs::to_bytes(&view_fungible::ApplicationCall::Balance)
    ...
    async fn send(&self, amount: u128, destination: AccountOwner) -> Result<(), Error> {
        ...
        self.transfer(view_fungible::ApplicationCall::Transfer(transfer))
            .await
    }

    /// Calls into the ViewFungible Token application to execute the `transfer`.
    async fn transfer(&self, transfer: view_fungible::ApplicationCall) -> Result<(), Error> {
        let transfer_bytes = bcs::to_bytes(&transfer).map_err(Error::InvalidTransfer)?;
...
pub enum Error {
    ...
    /// View_Crowd-funding application doesn't support any cross-chain effects.
    #[error("View_Crowd-funding application doesn't support any cross-chain effects")]
    EffectsNotSupported,

    /// View_Crowd-funding application doesn't support any cross-application sessions.
    #[error("View_Crowd-funding application doesn't support any cross-application sessions")]
    SessionsNotSupported,

    /// Failure to deserialize the initialization parameters.
    #[error("View_Crowd-funding campaign parameters are invalid")]
    InvalidParameters(bcs::Error),

    /// View_Crowd-funding campaigns can't start after its deadline.
    #[error("View_Crowd-funding campaign can not start after its deadline")]
    DeadlineInThePast,
    ...
    /// ViewFungible Token application did not execute the requested transfer.
    ...
    #[error("View_Crowd-funding campaign has not reached its target yet")]
    TargetNotReached,

    /// Can't cancel a campaign before its deadline.
    #[error("View_Crowd-funding campaign has not reached its deadline yet")]
    DeadlineNotReached,

    /// Can't cancel a campaign after it has been completed.
    #[error("View_Crowd-funding campaign has already been completed")]
    Completed,

    /// Can't pledge to or collect pledges from a cancelled campaign.
    #[error("View_Crowd-funding campaign has been cancelled")]
    Cancelled,
}
