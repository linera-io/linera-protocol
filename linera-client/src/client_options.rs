...
use linera_core::{
    client::{
        ...
        DEFAULT_MAX_EVENT_STREAM_QUERIES, DEFAULT_RECEIVED_LOG_PAGES_BEFORE_PACING,
...
pub struct Options {
    ...
    /// Number of full received-log pages to fetch back-to-back before pausing between pages.
    /// Serving a page makes the validator read one storage row per log entry, so an
    /// unthrottled walk of a long backlog degrades its read latency for every other client.
    /// Set to 0 to pace from the first page.
    #[arg(long, default_value_t = DEFAULT_RECEIVED_LOG_PAGES_BEFORE_PACING)]
...
impl Options {
    ...
    pub(crate) fn to_chain_client_options(&self) -> chain_client::Options {
        ...
        chain_client::Options {
            ...
            received_log_pages_before_pacing: self.received_log_pages_before_pacing,
