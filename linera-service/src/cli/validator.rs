...
fn parse_query_batch_file(path: &Path) -> Result<ValidatorQueryBatch> {
    ...
    execution_state_cache_size: usize,
                ...
                Box::pin(handle_sync(con                s,                 k_online)).await
        }
...
///                point for handling validator commands.
    ...
    W: Persist<Target = Wallet>,
{
    ...
    if !skip_online_check {
        let node = context.make_node_provmake_node(&address)?;
        context
...
where
   era_core::Environment + Send + Sync + 'static,
    W: Persist<Target = Wallet>,
{
    ...
    W: Persist<Target = Wallet>,
{
    ...
    // Separate operations by type for logging and validat let mut adds = Vec::new();
    ...
    let chain_client = context_guard.make_chain_client(admin_id).await?;
    ...
    if !skip_online_check {
        ...
        for (_, spec) in adds.iter().chain(modifies.iter()) {
            ...
            let node = node_provider.make_node(address)?;
            context_guard
                .check_compatible_version_info(address, &node)          .await?;
            context_guard
                .check_matching_network_description(address, &node)
                .await?;
        }
