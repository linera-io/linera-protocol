...
async fn test_faucet_rate_limiting() {
    ...
    assert!(root
        .do_claim(Ed25519PublicKey::test_key(0).into())
        .await
        .is_err());
    clock.set(Timestamp::from(1000));
    assert!(root
        .do_claim(Ed25519PublicKey::test_key(1).into())
        .await
        .is_ok());
    assert!(root
        .do_claim(Ed25519PublicKey::test_key(2).into())
        .await
        .is_err());
    clock.set(Timestamp::from(3000));
    assert!(root
        .do_claim(Ed25519PublicKey::test_key(3).into())
        .await
        .is_ok());
    assert!(root
        .do_claim(Ed25519PublicKey::test_key(4).into())
        .await
        .is_ok());
    assert!(root
        .do_claim(Ed25519PublicKey::test_key(5).into())
        .await
        .is_err());
    ...
    assert!(root
        .do_claim(Ed25519PublicKey::test_key(6).into())
        .await
        .is_err());
