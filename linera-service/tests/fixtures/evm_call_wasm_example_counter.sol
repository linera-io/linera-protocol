...
// The serde-json conversion code has been generated by hand.
//
// `CounterOperation`'s BCS serializer is also hand-written: the Rust enum
// carries `#[derive(linera_sdk::formats::StableEnum)]`, whose wire format
// prefixes each variant with a 4-byte ULEB128 tag derived from
// `Keccak256(variant_name)`, not the sequential `uint8` index that
// `serde-reflection`'s Solidity backend would emit by default.
...
contract ExampleCallWasmCounter {
    ...
    struct CounterOperation {
        // Only variant of the Rust enum: `Increment(u64)`.
        uint64 increment;
    }

    // Stable tag for `Increment`: `Keccak256("Increment")[..4]` interpreted
    // as a big-endian u32 with the top 5 bits masked to `00001`, i.e.
    // 0x0B21EABF (186_772_159). Its 4-byte ULEB128 encoding is BF D5 87 59.
    bytes constant COUNTER_OPERATION_INCREMENT_TAG = hex"BFD58759";

    function bcs_serialize_CounterOperation(CounterOperation memory input) internal pure returns (bytes memory) {
        return abi.encodePacked(COUNTER_OPERATION_INCREMENT_TAG, bcs_serialize_uint64(input.increment));
    }
