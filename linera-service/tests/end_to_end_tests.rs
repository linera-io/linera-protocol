...
impl Application {
    ...
    async fn increment_counter_value(&self, increment: u64) {
        let query_string = format!(
            "mutation {{  executeOperation(operation: {})}}",
            increment
        );
...
async fn test_end_to_end_counter() {
    ...
    let application_id = client
        .publish_and_create(contract, service, hex::encode(bcs::to_bytes(&original_counter_value).unwrap()), None)
        .await;
...
async fn test_end_to_end_counter_publish_create() {
    ...
    let application_id = client
        .create_application(bytecode_id, hex::encode(bcs::to_bytes(&original_counter_value).unwrap()), None)
        .await;
    let mut node_service = client.run_node_service(None, None).await;
