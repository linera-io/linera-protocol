...
async fn test_wasm_end_to_end_same_wallet_fungible(
    ...
    let expected_balances = state.accounts.into_iter().collect::<Vec<_>>();
...
#[cfg_attr(feature = "remote-net", test_case(RemoteNetTestingConfig::new(LeakChains) ; "remote_net_grpc"))]
#[test_log::test(tokio::test)]
async fn test_wasm_end_to_end_multi_chain_matching_engine_benchmark(
    config: impl LineraNetConfig,
) -> Result<()> {
    use std::collections::BTreeMap;
    ...
    let num_bid_chains = 50;
    let num_ask_chains = 50;
    ...
    let token0_mutations = bid_chains
        .iter()
        .map(|chain| {
            format!(
    ...
    let token1_mutations = ask_chains
        .iter()
        .map(|chain| {
            format!(
                "transfer(owner: {}, amount: \"1\", targetAccount: {})",
