...
async fn test_wasm_end_to_end_counter_no_graphql(config: impl LineraNetConfig) -> Result<()> {
    ...
    let raw_application = node_service
        .make_raw_application(&chain, &application_id)
        .await?;

    let query = serde_json::to_vec(&())?;
    let query = hex::encode(&query);

    let result = raw_application.raw_query(&query).await?;
    let counter_value: u64 = serde_json::from_slice(&result)?;
    assert_eq!(counter_value, original_counter_value);

    let operation = bcs::to_bytes(&increment)?;
    let operation = hex::encode(&operation);
    raw_application.raw_operation(&operation).await?;
