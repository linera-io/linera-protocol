...
use linera_rpc::grpc::api::{
    ...
    NotificationBatch, PushCertificateRequest, RelayPushRequest,
};
...
async fn an_absent_method_answers_unimplemented() -> anyhow::Result<()> {
    ...
    // The destination-facing stream, which is what a validator with an older binary is missing.
    // Opening it must be refused rather than hanging, since that refusal is the whole fallback
    // signal.
    let status = ValidatorNodeClient::connect(endpoint.clone())
        .await?
        .push_confirmed_certificates(tokio_stream::iter([PushCertificateRequest::default()]))
        .await
        .expect_err("a server that does not serve the method must refuse the call");
    assert_eq!(status.code(), Code::Unimplemented);

    // And the relay stream, which is what our own proxy is missing while it is still rolling.
    let status = ValidatorRelayClient::connect(endpoint)
        .await?
        .relay_confirmed_certificates(tokio_stream::iter([RelayPushRequest::default()]))
        .await
        .expect_err("a server that does not serve the method must refuse the call");
    assert_eq!(status.code(), Code::Unimplemented);
