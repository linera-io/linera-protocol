...
use crate::{
    grpc::api::CertificateRaw, HandleConfirmedCertificateRequest, HandleLiteCertRequest,
        ...
        while !missing.is_empty() {
            ...
            let mut received: Vec<ConfirmedBlockCertificate> =
                client_delegate!(self, download_certificates_by_heights, request)?
                    .certificates
                    .into_iter()
                    .map(|CertificateRaw { lite_cert, block }| {
                        let cert = bcs::from_bytes::<LiteCertificate>(&lite_cert)
                            .map_err(|_| NodeError::UnexpectedCertificateValue)?;

                        let block = bcs::from_bytes::<ConfirmedBlock>(&block)
                            .map_err(|_| NodeError::UnexpectedCertificateValue)?;
