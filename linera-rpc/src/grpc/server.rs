...
impl BatchForwarder {
    /// Spawns a single task and increments the in-flight counter.
    ...
    /// Waits for a task to complete and decrements the counter.
    /// Returns `Some(())` if a task completed, `None` if no tasks were pending.
    async fn wait_for_task(&mut self) -> Option<()> {
    ...
    fn spawn_batches(&mut self) {
        while !self.pending_notifications.is_empty() && self.tasks_in_flight < self.max_tasks {
    ...
    fn is_fully_drained(&self) -> bool {
        self.pending_notifications.is_empty() && self.tasks_in_flight == 0
    }
                ...
                result = receiver.recv() => {
                    match result {
                        Ok(notification) => {
                            ...
                            if forwarder.tasks_in_flight == 0
                               || (forwarder.pending_notifications.len() >= forwarder.batch_limit
                                   && forwarder.tasks_in_flight < forwarder.max_tasks) {
